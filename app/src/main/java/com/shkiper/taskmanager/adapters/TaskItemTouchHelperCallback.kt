...
class TaskItemTouchHelperCallback(val adapter: TaskAdapter,val swipeListener: (Task)->Unit
    ...
    private fun drawIcon(canvas: Canvas, itemView: View, dX: Float) {
        val icon = itemView.resources.getDrawable(R.drawable.ic_done_black_24dp, itemView.context.theme)
    ...
    private fun drawBackground(canvas: Canvas, itemView: View, dX: Float) {
        with(bgRect) {
            ...
            with(bgPaint) {
                color =
                    itemView.resources.getColor(R.color.color_green, itemView.context.theme)
            }
